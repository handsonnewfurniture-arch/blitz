    ...
    async def _run():
        ...
        if pipeline:
            ...
            if not summary:
                ...
                click.echo("Run a pipeline first: blitztigerclaw run pipeline.yaml")
                return
...
def board():
    ...
    if max_items == 0:
        ...
        click.echo("\n  Add pipelines: blitztigerclaw queue pipeline.yaml")
        return
...
def queue(file, var):
    ...
    click.echo(f"  Run next: blitztigerclaw work")
...
def work(limit, verbose):
    ...
    while True:
        ...
        if item is None:
            if processed == 0:
                ...
                click.echo("  Add pipelines: blitztigerclaw queue pipeline.yaml")
            else:
    ...
    if processed > 0:
        click.echo(f"\n  View board: blitztigerclaw board")
